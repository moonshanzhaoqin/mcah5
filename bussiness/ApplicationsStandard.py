...
# -*- coding:utf-8 -*-
# @Author:moon.shan#!/usr/local/bin/python
# -*- coding:utf-8 -*-
# @Author:moon.shan
...
class Applications():
    ...
    def configs(self,jwt,aid,loanAppId,companyId):
        print companyId
        print "-----------------------------------"
        self.url="/gw/borrow-api/v3/borrowers/%s/applications/%s/configs"%(companyId,loanAppId)
        self.headers['Authorization'] = jwt
    ...
    def getMaterials(self,jwt,companyId,appid):
        url2 = "/gw/borrow-api/v3/borrowers/%s/applications/%s/materials"%(companyId,appid)
    ...
    def PutMaterials(self,jwt,companyId,appid,initDataOne):
        self.url3 = "/gw/borrow-api/v3/borrowers/%s/applications/%s/materials?configCode=mcah5"%(companyId,appid)
        self.headers['Authorization'] = jwt
...
if __name__=='__main__':
    ...
    loanAppId = application.application(jwt,companyId)
    inputParams = application.configs(jwt,aid,loanAppId,companyId)
    getLoanInfo = application.getMaterials(jwt, companyId, loanAppId)
    inputLoanInfo = application.PutMaterials(jwt,companyId,loanAppId,getLoanInfo)
    uploadFile = application.upLoadDocuments(jwt,companyId,loanAppId)
